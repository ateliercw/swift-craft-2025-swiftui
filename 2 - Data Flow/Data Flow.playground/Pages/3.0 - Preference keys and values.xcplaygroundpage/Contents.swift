...
/*:
 If you want data to move up the hierarchy from a child view to the parent, then the tool to use is a preference key.

 First we define a preference key, which implements a function that takes the current value, and a proposed value and can write the updated value to the current value. This needs to used because many child views may try to set a preference key, so each view needs to be able to combine those values.
